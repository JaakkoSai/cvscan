...
def optimize_resume(resume_text: str, missing_keywords: list, hiring_company_name: str = "the target company", target_country: str = "International") -> dict:
    prompt = f"""
    ...
    2.  **Integrate Missing Keywords**: The candidate is missing: {missing_keywords}. Rewrite bullet points to naturally include these skills.
    3.  **Consolidate Skills**: Do NOT have both a "Core Competencies" and "Technical Skills" section if they repeat the same content. Merge them into one strong "Technical Skills" or "Core Competencies" section.
    4.  **Upgrade "Interests" / "Self Learning"**: If the resume has an Interests or Self Learning section, update it to include technologies relevant to {hiring_company_name} (e.g. if they use Azure, mention "Learning Azure Cloud Architecture").
    5.  **Impact-Driven**: Do not just stuff keywords. Rewrite weak bullet points to be results-oriented (e.g. "Used Python" -> "Leveraged Python to automate workflows, saving 10 hours/week").
    6.  **Full Rewrite**: Return the **ENTIRE** resume text with your improvements. Make it shine!
